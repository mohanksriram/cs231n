...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    z1 = X.dot(W1) + b1 #+ reg*np.sum(W1*W1) # N X H
    g1 = np.maximum(0, z1) # N X H
    z2 = g1.dot(W2) + b2 #+ reg*np.sum(W1*W1) # N X C
    scores = z2
    ...
    matching_scores = shift_scores[np.arange(len(y)),y]
  
    # Calculate the sofmax function
    exp_scores = np.exp(matching_scores)
    exp_sum = np.sum(np.exp(shift_scores), axis = 1)
    ...
    grads['W1'] = (X.T).dot(dG1)
    grads['W1'] += reg*W1
    grads['b1'] = np.sum(dG1, axis =0)
